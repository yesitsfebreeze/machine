...
Files matching patterns in `.proj/config/sections/mx.yaml` exclude list are not tagged:
...
Per-file limits from `.proj/config/sections/mx.yaml` (defaults):
- `anchor_per_file`: 3
- `warn_per_file`: 5
...
Project-level settings in `.proj/config/sections/mx.yaml`:
...
**IMPORTANT**: @MX tag descriptions MUST respect the `code_comments` setting from `.proj/config/sections/language.yaml`.
...
Before adding @MX tags, agents MUST read `.proj/config/sections/language.yaml` and use the `code_comments` value to determine the tag language.
