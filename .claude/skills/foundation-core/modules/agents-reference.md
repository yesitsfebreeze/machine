# Agents Reference - the machine Agent Catalog

Purpose: Complete reference catalog of the machine's 26 specialized agents with `{domain}-{role}` naming convention and 7-tier hierarchy.
...
Machine delegates ALL tasks to specialized agents. 26 agents organized in 7 tiers:
...
All the machine agents follow consistent naming:
...
Core command processors directly bound to Machine commands.

| Agent | Command | Purpose |
|-------|---------|---------|
| `workflow-project` | `the machine:0-project` | Project initialization and setup |
| `workflow-spec` | `the machine:1-plan` | EARS SPEC generation and planning |
| `workflow-ddd` | `the machine:2-run` | DDD ANALYZE-PRESERVE-IMPROVE execution |
| `workflow-docs` | `the machine:3-sync` | Documentation generation and synchronization |
...
|-------|---------|---------|
| `core-planner` | `the machine:2-run` Phase 1 | SPEC analysis and execution strategy |
...
Meta-generation agents for the machine development.
...
Use Case: When developing the machine itself (not for end-user projects)
...
Note: These are Claude Code built-in agents, not the machine custom agents.
...
1. Agent-First: Machine NEVER executes tasks directly. ALWAYS delegates via Agent()
...
| Foundation | foundation-core, foundation-cc, foundation-context, foundation-quality | Core principles and quality gates |
...
- [foundation-context](../..the machine-foundation-context/SKILL.md) - Token budget and session state
- [foundation-cc](../..the machine-foundation-cc/SKILL.md) - Claude Code configuration
...
Commands:
- `the machine:0-project` → `workflow-project`
- `the machine:1-plan` → `workflow-spec`
- `the machine:2-run` → `workflow-ddd`
- `the machine:3-sync` → `workflow-docs`
