...
description: >
  Provides the machine foundational principles including TRUST 5 quality framework,
...
Foundational principles and architectural patterns that power the machine's AI-driven development workflow.
...
Phase 1 SPEC (the machine:1-plan): workflow-spec generates EARS format. Output is .proj/specs/SPEC-XXX/spec.md. Execute /clear to save 45-50K tokens.

Phase 2 DDD (the machine:2-run): ANALYZE for requirements, PRESERVE for existing behavior, IMPROVE for enhancement. Validate with at least 85% coverage.

Phase 3 Docs (the machine:3-sync): API documentation, architecture diagrams, project reports.
...
Core Principle: Machine must delegate all work through Agent() to specialized agents. Direct execution bypasses specialization, quality gates, and token optimization. Proper delegation improves task success rate by 40 percent and enables parallel execution.
...
Phase Separation: Execute /clear between phases, after the machine:1-plan to save 45-50K, when context exceeds 150K, after 50+ messages.
...
Skills: cc-claude-md for CLAUDE.md with foundation patterns, cc-configuration for config with TRUST 5, cc-memory for token optimization, context7-integration for MCP integration.

Tools: AskUserQuestion for direct user interaction and clarification needs.

Commands: the machine:1-plan for SPEC-First Phase 1, the machine:2-run for DDD Phase 2, the machine:3-sync for Documentation Phase 3, the machine:9-feedback for continuous improvement, /clear for token management.
...
| "Delegation to an agent is overhead for simple tasks" | Machine is an orchestrator, not an implementer. Skipping delegation bypasses domain expertise and quality checks. |
...
- Machine executing implementation code directly instead of delegating to an agent
...
- [ ] Delegation chain traceable: Machine -> agent -> execution
