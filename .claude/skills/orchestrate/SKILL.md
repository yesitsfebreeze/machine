...
| `changes-requested` | Redo in progress: `SendMessage` sent via `redo`, awaiting the agent's response. Transient; becomes `running` once the agent re-engages. | shown, running |
...
Unfreezing is achieved by `approve` (fire now) or by `drop`. Editing a `frozen`
task updates its content only and leaves it frozen (see the transition rules
above) — edit is not an unfreeze.
...
imminent auto-fire is visible and overridable; show `running` and
`changes-requested` tasks as in-flight (a `changes-requested` task is a redo whose
`SendMessage` was sent and whose response is pending, so it carries no
time-to-fire); mark `frozen` tasks as paused; mark `untrusted` entries as needing
human review and NEVER show them with a time-to-fire (they do not auto-fire);
surface a task blocked on an unapproved or rejected dependency rather than hiding
it.
