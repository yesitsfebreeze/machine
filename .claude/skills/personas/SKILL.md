---
name: personas
description: Run the project's persona review panel in parallel against the last completed feature or fix. The panel is defined by /.machine/personas/ — each reviewer critiques from their own lens, then a synthesis pass identifies the top cross-cutting concerns and a ship verdict.
---
...
**data-driven** — it is defined by the files in `.machine/personas/`, not hard-coded
here, so it stays in sync as personas are added or revised.

## Step 1: Load the panel + the work under review

1. Read `.machine/personas.md` (the index) and **every** file in
   `.machine/personas/`. Each file is one reviewer: its name, role, what it
   scrutinizes, its standing concerns, and the question it always asks.
2. Read the project identity line and overarching goal from `/.machine/project.md`
   (and `/.machine/agent.md` if present). Capture as `{PROJECT_IDENTITY}` and
   `{PROJECT_GOAL}`.
