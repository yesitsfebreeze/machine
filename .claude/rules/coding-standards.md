...
| `ScheduleWakeup` | built-in | Harness built-in driver/main-loop tool (loop dynamic-pacing): re-invokes the main-loop driver after a delay (timed wakeup). NOT available to dispatched subagents — only the main-loop driver can call it, which is why a subagent inspecting its own tool registry will not find it |
