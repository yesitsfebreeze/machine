# Terms

The canonical names of the machine. This file is the **rename registry**, not a
dictionary — it exists so the main terms can be renamed in one clean grep-driven
sweep. Full definitions live once in `/.machine/glossary.csv` (the single source);
do not restate them here.

Each row: the canonical spelling, its kind, a one-line role, and where it lives.
To rename a term, change it here, in the glossary, and grep the codebase for the
old name.

## Rename plan

Fill the **New name** column (leftmost) for any term you want to rename, then hand it
back — each row becomes one verified `git grep` sweep + commit. Hit counts are
tracked-file occurrences (gitignored worktrees excluded). Leave a row blank to keep it.

### Project identity
| New name | Term | Hits | Files | Blast radius |
|----------|------|------|-------|--------------|
|          | the machine | 115 | 46 | name + `machine:` plugin namespace + plugin.json + marketplace + README badges |

### Coined names — clean grep-rename (distinctive, low collision)
| New name | Term | Hits | Files | What it is |
|----------|------|------|-------|------------|
|          | hub | — | — | coordination daemon (Rust binary; replaced the former `mesh.mjs`; state still in `.mesh/`) |
|          | drill | 263 | 27 | orchestrator surface (absorbs former `grill` vocabulary) |
|          | ledger | 146 | 41 | `.machine/sessions/` roster |
|          | git-fs / gitfs | 135 / 52 | 25 / 17 | branch-isolation layer (two spellings) |
|          | kern | 109 | 29 | memory daemon |
|          | personas | 96 | 31 | review panel |
|          | drill-first / drilling | 13 / 17 | — | driver mode (drill sub-vocabulary) |
|          | miner | 17 | 5 | spoken name for the implementation subagent |

### Collision-prone — sweep needs word-boundary/context review (common English words)
| New name | Term | Hits | Files | Risk |
|----------|------|------|-------|------|
|          | gate | 162 | 37 | "gate one" / "quality gate" + generic prose |
|          | factory | 111 | 14 | "factory agent" vs generic |
|          | mine | 99 | 21 | collides with English "mine"; also `mine/` dir + `/mine` skill |
|          | oil / /oil | 75 / 64 | 22 / 21 | "oil"/"oiled" verbs; target `/oil` to be safe |

### Not recommended to rename (platform/upstream/structural, not project brand)
`subagent` (Claude Code term) · `board`/`kanban` (in-plugin addon) · `glossary` ·
`improve.json` · `gate` (partly generic).

## Core identity

| Term | Kind | Role | Lives in |
|------|------|------|----------|
| the machine | identity | The portable, project-agnostic Claude Code setup | `.claude/` |
| the portable machine | layer | Identical-in-every-repo payload (skills/agents/hooks/rules) | `.claude/` |
| the project layer | layer | Per-repo specialization regenerated by `/oil` | `/.machine/` |
| kern | daemon | Durable per-cwd memory daemon (`mcp__kern__*`) | `.kern/` |
| hub | daemon | Per-cwd fleet-coordination daemon (`mcp__hub__*`); Rust binary replacing the former `mesh.mjs` | `hub/`, `.mesh/` (state dir name kept) |
| git-fs | layer | Branch-per-agent isolation + change history (`gitfs/<sid>` branches) | `.git-fs/` |

## Orchestration (the drill surface)

| Term | Kind | Role | Lives in |
|------|------|------|----------|
| the drill | surface | Drill-first main-loop driver (agent + skill) | `.claude/skills/drill/`, `.claude/agents/drill.md` |
| drill-first | mode | Refine a request one question at a time until valid before any spawn | `.claude/skills/drill/` |
| miner | surface | Spoken name for a dispatched implementation subagent | `.claude/skills/drill/` |
| plan agent | role | Sub-agent that writes the implementation plan | `.claude/skills/drill/` |
| implementation agent | role | Sub-agent that builds on its own git-fs branch | `.claude/skills/drill/` |
| ledger | artifact | Live roster of per-job entry-files (the set of files IS the roster) | `/.machine/sessions/` |
| gate one / gate two | gates | Human approval before implement / before merge | `.claude/skills/drill/` |
| board-trust | guard | Only driver-owned ledger entries may be acted on | `.claude/skills/drill/` |
| factory agent | role | Default agent owning one feature's full eight-stage lifecycle | `target.md` |

## Process skills

| Term | Kind | Role | Lives in |
|------|------|------|----------|
| /oil | skill | Re-index `/.machine/`; specialize the machine to this repo | `.claude/skills/oil/` |
| /mine | skill | Survey the mine graph + repo, slot best-fit addons | `.claude/skills/mine/` |
| /gate | skill | Format + lint + test + build pass/fail gate | `.claude/skills/gate/` |
| /personas | skill | Parallel persona review panel | `.claude/skills/personas/`, `/.machine/personas/` |
| /improve | skill | Rate-and-improve sweep driven by `improve.json` | `.claude/skills/improve/` |
| codex-review | skill | Advisory second-AI pass (never gates) | `.claude/skills/codex-review/` |

## Law & vocabulary

| Term | Kind | Role | Lives in |
|------|------|------|----------|
| Machine law | law | Always-applies rules (root cause, one clean impl, glossary discipline) | `.claude/agents/default.md` |
| Project law | law | Per-repo binding rules | `/.machine/agent.md` |
| glossary | artifact | Single source for full term definitions | `/.machine/glossary.csv` |
| mine graph | artifact | Curated kit of unregistered addons | `mine/` |
| improve.json | artifact | Outstanding-work list for the `/improve` loop | `/.machine/improve.json` |

## Pending reconciliation (resolve before the rename sweep)

Term divergences found across the docs — each must be unified before any rename:

- **git-fs branch name.** RESOLVED → canonical `gitfs/<sid>` (real plugin v3.1.2,
  verified on disk). Renamed across the drill layer (`drill/SKILL.md`, `agents/drill.md`,
  `agents/default.md`, agent-memory) and worktree dirs (`gitfs-<sid>`). `drill/<sid>`
  (orchestrator worktree) is intentionally separate and kept.
- **hub identity vs git-fs branch.** RESOLVED → unified to `gitfs/<sid>`. hub treats
  `agent_id` as an opaque principal (no format check), so by convention an agent's
  `agent_id` *is* its git-fs branch `gitfs/<sid>`. Updated in the comm-daemon SPEC (v1.1.0).
- **mesh.mjs → hub (Rust).** RESOLVED → the `mesh/mesh.mjs` Node daemon was replaced by
  the `hub` Rust binary (`hub/target/release/hub`, MCP server name `hub`, verbs `mcp__hub__*`).
  State stays in the repo-scoped `.mesh/` directory for data continuity.
- **git-fs version drift.** HANDOFF.md says v3.0.1; installed plugin is v3.1.2.
- **subagent spelling.** Canonical is one word **`subagent`** (matches Claude Code's
  official term; the law source `default.md` already prefers it). Source files
  (glossary, TERMS, default.md) are unified. ~320 `sub-agent` occurrences remain
  across `.claude/**` and `.machine/**` docs — repo-wide sweep queued in improve.json.
