...
**Input**:
```python
from workflow_testing import DDDManager, TestSpecification, TestType
...
**Input**:
```python
from workflow_testing import AutomatedCodeReviewer
  ...
  "auto_fixable": [
    {
      "file": "src/auth/service.py",
      "fix": "Add type hints",
      "apply_command": "workflow fix --file src/auth/service.py --type hints"
    }
  ]
}
```
...
```python
from workflow_testing import QualityGateManager
...
```bash
# 전체 워크플로우 실행
workflow execute --project /project/src --mode full

# 개별 컴포넌트 실행
workflow debug --file app.py --error "AttributeError"
workflow refactor --directory src/ --max-risk medium
workflow profile --target function_name --types cpu,memory
workflow test --spec user_auth.spec --mode ddd
workflow review --project /project/src --trust-score-min 0.8

# CI 모드
workflow ci --commit abc123 --quality-gates strict
```
