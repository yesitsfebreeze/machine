...
TRUST 5 is the machine's comprehensive quality assurance framework enforcing five pillars:
...
- the machine:2-run → Enforces ≥85% coverage
...
```bash
# 1. Generate SPEC
the machine:1-plan "Implement user authentication with JWT tokens"
...
# Validate skill
result = validator.validate_skill(".claude/skillsthe machine-foundation-core")
...
if not result["valid"]:
 ...
 # Auto-split
 validator.auto_split_skill(".claude/skillsthe machine-foundation-core")
...
# Usage
loader = ProgressiveContentLoader(".claude/skillsthe machine-foundation-core")
...
Skills:
- foundation-modular-system - File organization patterns
- foundation-token-optimization - Content efficiency
- cc-skill-factory - Skill creation with progressive structure
...
Commands:
- the machine:1-plan - Generate SPEC with progressive detail
- the machine:3-sync - Create docs with layered structure
