...
Purpose: Security policies, execution constraints, and Git workflow strategies governing the machine agent behavior.
...
Machine operates under strict execution rules:
...
| `Skill()` | Knowledge invocation | `Skill("foundation-core")` |
...
Machine automatically adjusts Git workflow based on `config.json` settings.
...
Machine's Behavior (prompt_always=true):
1. When running `the machine:1-plan`, user prompted: "Create branch?"
...
Machine's Behavior (prompt_always=false):
...
2. Running `the machine:2-run`: DDD commits + auto push
3. Running `the machine:3-sync`: Doc commits + suggest PR creation (user choice)
...
Machine's Behavior (prompt_always=false, auto_enabled=false):
1. When running `the machine:1-plan`, user prompted once: "Enable automatic branch creation?"
...
Machine's Behavior (prompt_always=false, auto_enabled=true):
...
2. Running `the machine:2-run`: DDD commits + auto push (to feature branch)
3. Running `the machine:3-sync`: Doc commits + prepare PR
...
Machine's Behavior (prompt_always=false, auto_enabled=false):
1. When running `the machine:1-plan`, user prompted once: "Enable automatic branch creation and Draft PR creation?"
...
Execute `/clear` after `the machine:1-plan` completion in ALL modes.
...
1. Immediately after `the machine:1-plan` (mandatory)
...
- Create documentation in `.proj/` directory
...
This comprehensive set of execution rules ensures that the machine operates securely, efficiently, and in compliance with industry standards while maintaining high quality output.
