...
1. Immediately after the machine:1-plan (saves 45-50K)
...
def restore_minimal_context(summary: dict):
 ...
 load_file(f".proj/specs/{summary['current_spec']}/spec.md")
...
class SelectiveFileLoader:
 ...
 f".proj/specs/{context['spec_id']}/spec.md"
 ]
 ...
 files = [f".proj/specs/{context['spec_id']}/spec.md"]
...
def load_history():
 load_file(".proj/conversation_history.json") # 500K+ tokens
```
...
Skills:
- foundation-delegation-patterns - Context passing
- foundation-progressive-disclosure - Content structuring
- cc-memory - Context persistence

Commands:
- /clear - Context reset (mandatory after the machine:1-plan)
- /context - Check current token usage
- the machine:1-plan - SPEC generation (30K budget)
- the machine:2-run - DDD implementation (180K budget)
- the machine:3-sync - Documentation (40K budget)

Memory:
- Skill("foundation-core") modules/token-optimization.md - Optimization strategies
- .proj/config/config.json - Budget configuration
